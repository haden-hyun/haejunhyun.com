...
작업·프로덕션 브랜치 모두 `v5` (Cloudflare가 이 브랜치만 배포한다).
...
| 파일               | 역할                                                                |
| ------------------ | ------------------------------------------------------------------- |
| `DESIGN-SYSTEM.md` | **디자인의 현재 상태와 기각 기록.** 색·서체를 건드리기 전 §5 필독      |
| `README.md`        | 배포 구조                                                           |

이 둘이 전부다. 계획서·핸드오프·이력 문서를 새로 만들지 않는다 — 디자인은
`DESIGN-SYSTEM.md` 한 곳에 현재 상태로만 적고, 경위는 git 히스토리가 갖는다.

**주석은 핵심 기능·커스텀 로직에만, 한 줄로.** `.scss`/`.yaml`/`.tsx` 어디든 동일 — "왜
이 값을 골랐는지" 서사·비교·근거 나열은 쓰지 않는다. 함정만 `주의:` 한 줄. 히스토리·날짜·
세션 서사·디자인 근거는 주석에 남기지 말고 DESIGN-SYSTEM.md로 보낸다.
...
8. **콜아웃 형태는 Obsidian 규격을 유지한다.** 아이콘·둥근 모서리·좌측 바를 걷어내는
   "인쇄 조판" 안은 시도 후 되돌렸다. 우리가 정의하는 건 **색이지 형태가 아니다.**
9. **패널 라벨 5종(SPECS/CATALOG/TOC/Backlinks/Graph)은 한 규칙으로 통일한다.**
   패널별로 색·조판을 나누면 라벨이 콘텐츠보다 시끄러워진다 — 시도 후 되돌림.
10. **좌우 여백의 단일 소스는 `.page`다.** 폭 계산에 `vw`를 쓰지 말 것 — 세로
    스크롤바 폭을 포함해 좌우가 어긋난다.
...
- 설계가 가정한 노트 수(129) ↔ 실측 90 → **통계는 `allFiles` 런타임 집계, 하드코딩 금지**
...
- **bold는 색이 아니라 재질/무게(`--dark`)로 구분한다.** 인라인 코드(틸 고정)·하이라이트
  (그린/골드, 형광펜 관습)는 예외로 색을 쓴다 — 세 축과는 색상 27° 이상 벌려 둔다(DESIGN-SYSTEM §4.2).
