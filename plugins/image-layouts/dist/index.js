...
var carousel_inline_default = 'document.addEventListener("nav",()=>{let v=document.querySelectorAll(\'.il-carousel[data-needs-init="true"]\');for(let l of v){let e=l.querySelector(".il-viewport");if(!e)continue;let s=Array.from(e.querySelectorAll(".il-item"));if(s.length<2)continue;let d=Array.from(l.querySelectorAll(".il-thumb")),c=()=>Math.round(e.scrollLeft/e.clientWidth),i=()=>{let t=c();d.forEach((n,r)=>n.classList.toggle("is-active",r===t))},a=t=>{let n=s[Math.max(0,Math.min(s.length-1,t))];n&&(e.scrollTo({left:n.offsetLeft-e.offsetLeft}),i())},u=(t,n,r,m)=>{let o=document.createElement("button");o.type="button",o.className=`il-nav ${t}`,o.setAttribute("aria-label",n),o.textContent=r,o.addEventListener("click",m),l.appendChild(o),window.addCleanup(()=>{o.removeEventListener("click",m),o.remove()})};u("il-prev","\\uC774\\uC804 \\uC774\\uBBF8\\uC9C0","\\u2039",()=>a(c()-1)),u("il-next","\\uB2E4\\uC74C \\uC774\\uBBF8\\uC9C0","\\u203A",()=>a(c()+1)),d.forEach((t,n)=>{let r=()=>a(n);t.addEventListener("click",r),window.addCleanup(()=>t.removeEventListener("click",r))}),e.addEventListener("scroll",i,{passive:!0}),window.addCleanup(()=>e.removeEventListener("scroll",i)),i(),l.removeAttribute("data-needs-init")}});\n';
