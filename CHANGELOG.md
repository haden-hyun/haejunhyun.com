...
**항목 색은 블루로 통일했다.** brick·chitlins도 후보였고 대비는 셋 다 AA를 넘지만,
둘은 화면에서 각각 다른 것을 망가뜨린다 — **chitlins는 TOC 활성 표시와 같은 색이라
"지금 읽는 위치"가 사라지고**, brick은 페이지당 1회여야 할 CTA 색이라 사이드바 수십
줄에 깔면 CTA를 찾을 수 없게 된다. 블루는 넷 다 링크라는 사실과 맞고, SPECS의 topic이
이미 블루라 변경도 가장 적었다.

⚠️ 이 과정에서 `noteCatalog.scss`의 `--gray` 지정이 남아 새 규칙을 되살려 덮는 걸
확인했다. 항목 색은 `custom.scss` 한 곳에 두고 플러그인 쪽엔 형태만 남긴다.
